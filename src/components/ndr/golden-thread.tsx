...
export function GoldenThread({
  ...
  hops?: ThreadHop[];
  loading?: boolean;
  /** Vertical stacks the hops for a side column; horizontal is the wide rail. */
  orientation?: "horizontal" | "vertical";
}) {
  const vertical = orientation === "vertical";
  if (!hops.length) {
    return (
      <div className={cn("text-xs text-muted-foreground", vertical ? "p-3" : "ndr-panel px-3 py-4")}>
  ...
  return (
    <div className={cn(vertical ? "flex h-full min-h-0 flex-col" : "ndr-panel px-3 pb-2.5 pt-2")}>
      <div className={cn("shrink-0", vertical ? "mb-2" : "mb-2 flex items-baseline gap-2")}>
        <h3 className="text-xs font-semibold uppercase tracking-wide">The Golden Thread</h3>
        <p className={cn("text-[10px] text-muted-foreground", vertical ? "" : "truncate")}>
      ...
      <ol
        className={cn(
          "scroll-thin flex gap-0",
          vertical ? "min-h-0 flex-1 flex-col overflow-y-auto pr-0.5" : "items-stretch overflow-x-auto pb-1"
        )}
      >
        {hops.map((hop, i) => {
          ...
          return (
            <li
              key={hop.id}
              className={cn("flex min-w-0", vertical ? "flex-col" : "flex-1 items-stretch")}
            >
              <div
                className={cn(
                  "group relative flex-1 rounded-md border transition",
                  vertical ? "px-2 py-1.5" : "min-w-[128px] px-2.5 py-2",
                  hop.stale
              ...
              >
                <div className={cn("flex items-center gap-1.5", vertical ? "mb-0.5" : "mb-1")}>
                  <Icon
                ...
                {/* Stacked in a column the figure and its caption share a line;
                    four stacked lines per hop would not fit seven of them. */}
                {vertical ? (
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="truncate text-[9px] text-muted-foreground">
                      {hop.ts ? relativeTime(hop.ts) : hop.sub}
                    </span>
                  </div>
                ) : (
                  <>
